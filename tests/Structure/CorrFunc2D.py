...
analysis = mdstuff.structure.CorrFunc2D(
    x_function=mdstuff.structure.InterCompoundDistance(
        selection="resname SQU", compound="residues"
    ),
    x_bounds=(0.0, 50.0),
    x_bin_width=1.0,
    y_function=mdstuff.structure.InterCompoundDistance(
        selection="type CTL3", compound="residues",
    ),
    y_bounds=(0.0, 50.0),
    y_bin_width=1.0,
)
with warnings.catch_warnings():
    ...
    universe = (
        mdstuff.NAMDUniverse(psf, dcds).add_analyses(analysis).run_analyses(stop=100)
    )
