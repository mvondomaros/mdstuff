...
class PDens(RunningHist):
    ...
    def __init__(
        self, function: StructureFunction, bounds: Tuple[float, float], bin_width: float
    ) -> None:
        """
...
class PDens2D(RunningHist2D):
    ...
    def get(self, centers: bool = False) -> Tuple[np.ndarray, np.ndarray, np.ndarray]:
        ...
        n = np.sum(counts)
        if n > 0.0:
            counts /= n * self.x_bins.bin_width * self.y_bins.bin_width
        if centers:
...
class CorrFunc2D(PDens2D):
    ...
    def get(self, centers: bool = False) -> Tuple[np.ndarray, np.ndarray, np.ndarray]:
        ...
        hxy /= np.sum(hxy) * self.x_bins.bin_width * self.y_bins.bin_width
        counts -= hxy
        if centers:
            return counts, self.x_bins.centers.copy(), self.y_bins.centers.copy()
        else:
